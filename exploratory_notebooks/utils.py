...
def Neutrsophic_Image(img: np.ndarray, inversion: bool = False):
    """
...
def alpha_mean_beta_enh(img: np.ndarray):
    ...
    while True:
        betaI_entropy, betaT, betaI = \
            guo2009_ns_transform(img, enl, alpha, beta)
...
def rashno_ns_mapper(g: np.ndarray):
    ...
    """
    # Step 2
    NS_im = Neutrsophic_Image(g, inversion=True)
    T = NS_im[:, :, 0]
    T_shape = T.shape
    # Step 3
...
def get_rpe_line(
    ...
    r_window: int = 50, u_window: int = 10
):
    """
    ...
    graph_, nodes, indices = pixel_graph(
        ...
        weights_u=U, ilm_image=ilm_image, connectivity=2, purpose='rpe_line'
    )
...
def get_retina_mask(ilm_image_fill: np.ndarray, rpe_image_fill: np.ndarray):
    """
...
def rpe_flattening(
    ...
    # Correct the indices positions
    indx = np.argsort(rpe_line[1])
    # Inverted to see them "from below"
    original_line = rpe_image.shape[0] - rpe_line[0][indx]
    ...
    for it in range(iterations):
        ...
        l_rpe = len(rpe)
        kernel = np.ones(k_size)/(k_size**2)
    ...
    return new_rpe, original_line, indx, pick_start, pick, pick_end


def clustering(img: np.ndarray, mask: np.ndarray, n_clusters: int = 5):
    """
    Args:
...
def select_seeds(
    img: np.ndarray, clust_result: np.ndarray, FN: int = 500, TN: int = 700
):
    """
    ...
    Args:
        ...
        FN (int, optional): [description]. Defaults to 500.
        TN (int, optional): [description]. Defaults to 700.
    Returns:
    ...
    # Fluid Seeds
    fluid_condition = \
        (clust_result == indices[0]) | (clust_result == indices[1])
    ...
    return fluid_seeds, retina_seeds


def Bpq(
    values0: np.ndarray, values1: np.ndarray, distances: np.ndarray,
    scaling: int = 2, sigma: float = (30/255)
):
    """
...
def image_from_minimum_cut(partition: tuple, shape: tuple, nodes: np.ndarray):
    ...
    segmentation = np.zeros(shape[0] * shape[1])
    obj_nodes = np.array(list(object_)[1:])-1
    bkg_nodes = np.array(list(background)[:-1])-1
    segmentation[nodes[obj_nodes]] = 2
    segmentation[nodes[bkg_nodes]] = 1
